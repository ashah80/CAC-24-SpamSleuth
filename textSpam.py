...
def predict_spam(text, baseline_model, model_1, model_2, model_3, tfidf_vec):
    """
    ...
    model_1: Custom Vec-Embedding model
    ...
    # Debug information
    print("\nModel Input Shapes:")
    print(f"Model 1 input shape: {model_1.input_shape}")
    ...
    # Custom Vec-Embedding Model prediction
    model1_pred = float(model_1(text_array, training=False)[0])
    results['Custom-Vec-Embedding'] = {
        'is_spam': bool(model1_pred > 0.5),
        'confidence': float(abs(model1_pred - 0.5) * 2 * 100)
    }
...
def print_spam_predictions(text):
    ...
    text (str): The input text message to classify
    """
    predictions = predict_spam(text, baseline_model, model_1, model_2, model_3, tfidf_vec)
...
while True:
    ...
    if message == "break":
        break
    print_spam_predictions(message)
